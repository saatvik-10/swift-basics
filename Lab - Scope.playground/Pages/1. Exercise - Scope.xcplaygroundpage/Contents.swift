...
//print("The value of foo is \(foo)")
// The constant 'foo' is declared inside the for loop and it's scope is limited inside that only. Accessing it outside of the for loop is not possible.
...
// In this case, the variable x is defined globally (global scope) and is modified inside a for loop as x can be accessed inside as well (local scope). In the previous case, the foo constant is declared locally and after that we are trying to access it globally which is nit possible as scope of foo is destroyed as soon as for loop ends.
